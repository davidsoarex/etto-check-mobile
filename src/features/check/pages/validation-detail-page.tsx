...
export function ValidationDetailPage() {
  ...
  const photoItems =
    detail?.tasks
      .filter((task) => task.photo?.id != null)
      .map((task) => ({
  ...
  const { entryFor, retryPreview, lightbox, openLightbox, closeLightbox } = useEcheckPhotoPreviews(
          ...
          <PortalSectionCard title="Fotos das tarefas" description="Confira cada foto antes de validar.">
            <div className="divide-y divide-slate-100">
              {detail.tasks.map((task) => {
                ...
                return (
                  <div key={task.id ?? `task-${task.title}`} className="space-y-3 px-4 py-4">
                    <div>
