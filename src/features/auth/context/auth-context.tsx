...
const MANAGE_TARGETS_KEY = 'etto.echeck.canManageCheckTargets'

function applyProfile(profile: EcheckPortalProfile) {
  const canManageCheckTargets = profile.canManageCheckTargets === true
  ...
  localStorage.setItem(MANAGE_TARGETS_KEY, canManageCheckTargets ? '1' : '0')
  return {
...
export function AuthProvider({ children }: PropsWithChildren) {
  ...
  const [canManageCheckTargets, setCanManageCheckTargets] = useState(
    localStorage.getItem(MANAGE_TARGETS_KEY) === '1',
  )
  ...
  useEffect(() => {
    ...
    void fetchEcheckMe(portalToken)
      .then((me) => {
        ...
        setCanManageCheckTargets(next.canManageCheckTargets)
